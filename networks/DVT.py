...
# ────────────────────────────────
# BEV ➜ RV (Descartes → Sphere)
# ────────────────────────────────
# class BEV2RV(nn.Module):
#     """
#     LiDAR 센서 원점 좌표계 기반  BEV → RV 변환
#       • bev_feat : (B, C, 512, 512)
#       • bev_z_bin: (B, 1, 512, 512) ─ Quantize+VoxelMaxPool 결과(지면 기준 ❌, 센서 기준 ✔️)
#       → rv_feat  : (B, C, 64, 2048)
#     """
...
#         z_range=(-4.0, 2.0),  # 센서 기준 z 범위  [m]
#         z_bins=30,  # desc._shape[2]
...
#         # ─ BEV 평면 좌표 버퍼 ─
#         x_lin = torch.linspace(self.xmin, self.xmax, self.W_b)
#         y_lin = torch.linspace(self.ymax, self.ymin, self.H_b)
#         xg, yg = torch.meshgrid(y_lin, x_lin)  # (H_b, W_b)
#         self.register_buffer("xg", xg)
#         self.register_buffer("yg", yg)

#     @torch.no_grad()
#     def forward(self, bev_feat: torch.Tensor, bev_z_bin: torch.Tensor) -> torch.Tensor:
#         """
#         bev_feat : (B, C, 512, 512)
#         bev_z_bin: (B, 1, 512, 512)  ─ bin index (0‥29)
#         """
#         B, C, _, _ = bev_feat.shape
#         device = bev_feat.device
#         x, y = self.xg.to(device), self.yg.to(device)  # (H_b, W_b)

#         # 1) bin → z_rel [m]  (센서 원점 기준)
#         dz = (self.z_max - self.z_min) / self.z_bins
#         z_rel = bev_z_bin.squeeze(1) * dz + (self.z_min + dz / 2)  # (B, H_b, W_b)

#         rho = torch.sqrt(x**2 + y**2) + 1e-6  # (H_b, W_b)
#         phi_const = torch.atan2(y, x)  # (H_b, W_b)

#         rv = torch.zeros(B, C, self.H_r, self.W_r, device=device)

#         for b in range(B):
#             # 2) 고도각 θ(↓ 음수)·방위각 φ 계산
#             theta = torch.atan2(z_rel[b], rho)  # ※ z_rel 그대로 사용
#             col = ((phi_const - self.phi_min) / (self.phi_max - self.phi_min) * (self.W_r - 1)).long()
#             row = ((self.theta_max - theta) / (self.theta_max - self.theta_min) * (self.H_r - 1)).long()

#             valid = (col >= 0) & (col < self.W_r) & (row >= 0) & (row < self.H_r)
#             if not valid.any():
#                 continue
