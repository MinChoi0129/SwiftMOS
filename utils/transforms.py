...
class Transformation:
    ...
    def __call__(self, pcloud, labels):
        ...
        out = (pcloud, labels) if self.inplace else (np.array(pcloud, copy=True), np.array(labels, copy=True))
        return out
...
class Crop(Transformation):
    def __init__(self, dims=(0, 1, 2), fov=((-5, -5, -5), (5, 5, 5)), eps=1e-4):
        ...
        for i, (min, max) in enumerate(zip(*fov)):
            assert min < max, f"Field of view: min ({min}) < max ({max}) is expected on dimension {i}."
    ...
    def __call__(self, pcloud, labels):
        ...
        for i, d in enumerate(self.dims):  # Actually a bug below, use d in pc not i!
            temp = (pc[:, d] > self.fov[0][i] + self.eps) & (pc[:, d] < self.fov[1][i] - self.eps)
...
class Voxelize(Transformation):
    ...
    def __call__(self, pcloud, labels):
        ...
        _, ind = np.unique((pc_shift / self.voxel_size).astype("int"), return_index=True, axis=0)
