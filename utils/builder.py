...
def get_scheduler(optimizer, pOpt, per_epoch_num_iters):
    ...
    if pOpt.schedule.type == "OneCycle":
        scheduler = lr_scheduler.OneCycleLR(
            optimizer,
            max_lr=pOpt.schedule.base_lr,
            epochs=num_epoch,
            steps_per_epoch=per_epoch_num_iters,
            pct_start=pOpt.schedule.pct_start,
            anneal_strategy="cos",
            div_factor=25,
            final_div_factor=pOpt.schedule.base_lr / pOpt.schedule.final_lr,
        )
        return scheduler
...
def get_optimizer(pOpt, model):
    if pOpt.optimizer.type in ["adam", "adamw"]:
        optimizer = AdamW(params=model.parameters(), lr=pOpt.optimizer.base_lr, weight_decay=pOpt.optimizer.wd)
        return optimizer
